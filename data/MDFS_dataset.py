...
class MDFSDataset(data.Dataset):
    """
    ...
    def __init__(self, opt, mode='train'):
        ...
        for deg_type in opt["distortion"]:
            ...
            # if deg_type == "rainy":
                ...
                # High rain density
                GT_H = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, "RainTrainH", "GT")
                )
                LR_H = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, "RainTrainH", "LQ")
                )

                # Low rain density
                GT_L = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, "RainTrainL", "GT")
                )
                LR_L = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, "RainTrainL", "LQ")
                )
            ...
            n = os.listdir(opt["first_stage_result"])
    ...
    def __getitem__(self, index):
        ...
        # get prediction image from first stage model
        FS_result_path = self.distortion[deg_type][2][index]
        img_FS = util.read_img(
        ...
        if self.opt["phase"] == "train":
            ...
            img_FS = img_FS[rnd_h : rnd_h + self.size, rnd_w : rnd_w + self.size, :]

            # augmentation - flip, rotate
            img_LQ, img_GT, img_FS = util.augment(
                [img_LQ, img_GT, img_FS],
                self.opt["use_flip"],
                self.opt["use_rot"],
                mode=self.opt["mode"],
            )
        ...
        if self.opt["color"]:
            ...
            img_FS = util.channel_convert(img_FS.shape[2], self.opt["color"], [img_FS])[0]
        ...
        img_FS = torch.from_numpy(np.ascontiguousarray(np.transpose(img_FS, (2, 0, 1)))).float()
