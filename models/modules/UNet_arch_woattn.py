...
class UNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, ch_mult=[1, 2, 4, 4], 
        ...
        # self.init_conv = default_conv(in_nc*in_ch_scale, nf, 3)
...
if __name__ == '__main__':
    model = UNet(in_nc=3, out_nc=3, nf=128, ch_mult=[1, 2, 4, 4], context_dim=512, use_daclip_context=False, in_ch_scale=1)
    input = torch.randn((1, 3, 257, 257))
    output = model(input)
    print(output.shape)