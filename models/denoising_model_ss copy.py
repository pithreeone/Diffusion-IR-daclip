...
class DenoisingModelSS(BaseModel):
    def __init__(self, opt):
        ...
        self.fs_model = networks.define_G(opt).to(self.device)
        self.ss_model = networks.define_G(opt).to(self.device)
        if opt["dist"]:
            self.ss_model = DistributedDataParallel(
                self.ss_model, device_ids=[torch.cuda.current_device()]
            )
        else:
            self.ss_model = DataParallel(self.model)
        # print network
        # self.print_network()
        self.load_fs()
        self.load_ss()
    ...
    def test(self, sde=None, mode='posterior', save_states=False):
        ...
        with torch.no_grad():
            if mode == 'sde':
                ...
                fs_output = sde.reverse_sde(self.state, save_states=save_states, text_context=self.text_context, image_context=self.image_context)
