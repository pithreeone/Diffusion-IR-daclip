...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        if need_GT:
            ...
            test_results["psnr_fs"].append(psnr_fs)
            ...
            if len(gt_img.shape) == 3:
                if gt_img.shape[2] == 3:  # RGB image
                    ...
                    logger.info(
                        "img{:3d}:{:15s} - PSNR: {:.6f} dB; SSIM: {:.6f}; LPIPS: {:.6f}; FS-PSNR: {:.6f}; PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}.".format(
                            i, img_name, psnr, ssim, lp_score, psnr_fs, psnr_y, ssim_y
                        )
                    )
            else:
                logger.info(
    ...
    avg_psnr_fs = sum(test_results["psnr_fs"]) / len(test_results["psnr_fs"])
    logger.info(
        "----Average PSNR/SSIM results for {}----\n\tPSNR: {:.6f} dB; SSIM: {:.6f}, PSNR (FS): {:.6f}\n".format(
            test_set_name, ave_psnr, ave_ssim, avg_psnr_fs
        )
    )
